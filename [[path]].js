...
async function zone(env,u){
  ...
  for(let i=0;i<days;i++){
    ...
    try{
      const q=`query($s:Time!,$e:Time!){viewer{zones{httpRequestsAdaptiveGroups(limit:10000,filter:{datetime_geq:$s,datetime_leq:$e,requestSource:"eyeball"}){count sum{edgeResponseBytes visits} dimensions{datetimeHour clientCountryName clientDeviceType edgeResponseStatus coloCode cacheStatus}}}}}`;
      const data=await gql(q,{s:start.toISOString(),e:end.toISOString()},token);
      const zones=data.viewer?.zones||[];
      for(const z of zones) httpRows.push(...(z.httpRequestsAdaptiveGroups||[]));
